...
public class GoogleCloudStorageService : IStorageService
{
    ...
    public GoogleCloudStorageService(IOptionsSnapshot<StorageOptions> options)
    {
        if (options == null)
    ...
    public Task<Uri> GetDownloadUriAsync(string path, CancellationToken cancellationToken = default)
        => Task.FromResult(new Uri($"https://storage.googleapis.com/{_bucketName}/{CoercePath(path).TrimStart('/')}"));
    ...
    private static string CoercePath(string path) => path.Replace('\\', '/');
}
